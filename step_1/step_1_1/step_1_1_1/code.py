...
DEFAULT = {
    ...
    "role": "student",
    "notes_played": [],
    "faculties": {"listening": 1, "discipline": 0}
}
CHOICES = {
    "1": "Do", "2": "Re", "3": "Mi", "4": "Fa",
...
def meditate(state):
    while True:
        ...
        c = input("> ").strip()
        if c == "1":
            save_state(state); print("State saved with discipline.")
        elif c == "2":
            return
        elif c == "3":
            print("Pick 1–7 to test notes (Do–Ti). 8 opens this menu.")
        elif c == "4":
            save_state(state); print("Final save complete. Session ended."); sys.exit(0)
...
def main():
    ...
    while True:
        print("\n# Rhythm — The Trial Room\nSelect a note to test:")
        for k in "12345678":
            print(f"{k}. {CHOICES[k]}")
        pick = input("> ").strip()
        if pick not in CHOICES:
            print("Invalid choice. Select 1–8."); continue
        label = CHOICES[pick]
        if label == "Meditate":
            meditate(s); continue
        s["notes_played"].append(label)
        s["faculties"]["discipline"] = s["faculties"].get("discipline", 0) + 1
        save_state(s)
        print(f"\n[{label}]\n{sections.get(label, '')}\n(Progress recorded.)")
